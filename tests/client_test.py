...
@pytest.mark.verifies_requirement("6.2")
...
@pytest.mark.verifies_requirement("F-API-000") # General API communication
...
@pytest.mark.verifies_requirement("F-Client-001")
...
@pytest.mark.verifies_requirement("F-Main-001")
...
@pytest.mark.asyncio
@pytest.mark.verifies_requirement("F-NATS-003")
...
@pytest.mark.asyncio
@pytest.mark.verifies_requirement("F-NATS-002")
...
@pytest.mark.asyncio
@pytest.mark.verifies_requirement("F-NATS-005")
...
@pytest.mark.verifies_requirement("F-Client-003")
...
@pytest.mark.verifies_requirement("F-Client-002")
def test_device_agent_collect_telemetry_psutil_not_installed(mock_device_agent_dependencies, caplog):
    ...
    with patch("equus_express.edge_device_controller.psutil", new=None): # Force psutil to be None
        with caplog.at_level(logging.WARNING):
            ...
            assert telemetry["system_info"]["platform"] == platform.system() # Should still get basic platform info
    ...
    with patch("equus_express.edge_device_controller.psutil", MagicMock()), \
        ...
        with caplog.at_level(logging.ERROR):
            with patch("equus_express.edge_device_controller.DeviceAgent._get_cpu_usage", side_effect=RuntimeError("CPU error")):
                ...
                assert telemetry["system_metrics"]["cpu_usage_percent"] == 0.0 # Default value on error. # No change
        ...
        with caplog.at_level(logging.ERROR):
            with patch("equus_express.edge_device_controller.DeviceAgent._get_memory_usage", side_effect=RuntimeError("Memory error")):
                telemetry = agent._collect_telemetry()
                assert "memory_total_mb" in telemetry["system_metrics"] # Check for specific key as per change
                assert telemetry["system_metrics"]["memory_total_mb"] == 0.0 # Default value on error. # No change
        ...
        with caplog.at_level(logging.ERROR):
            with patch("equus_express.edge_device_controller.DeviceAgent._get_disk_usage", side_effect=RuntimeError("Disk error")):
                telemetry = agent._collect_telemetry()
                assert "disk_total_gb" in telemetry["system_metrics"] # Check for specific key as per change
                assert telemetry["system_metrics"]["disk_total_gb"] == 0.0 # Default value on error. # No change
        ...
        with caplog.at_level(logging.ERROR):
            with patch("equus_express.edge_device_controller.DeviceAgent._get_temperature", side_effect=OSError("Temp file error")):
                ...
                assert telemetry["system_metrics"]["temperature_celsius"] == 0.0 # Default value on error. # No change
        ...
        with caplog.at_level(logging.ERROR):
            with patch("equus_express.edge_device_controller.SecureAPIClient._get_ip_address", side_effect=socket.gaierror("IP error")):
                ...
                assert telemetry["network_info"]["ip_address"] == "unknown" # Default value on error. Should be "unknown" as per controller.
...
def test_device_agent_collect_telemetry_structure(mock_device_agent_dependencies):
    ...
    ): # No alias for the context manager here
        telemetry = agent._collect_telemetry()
...
def test_device_agent_collect_telemetry_structure(mock_device_agent_dependencies):
    ...
    with (
        ...
        patch.object(agent.client, '_get_ip_address', return_value=MOCK_IP_ADDRESS), # Mock method on the client instance
        patch("equus_express.edge_device_controller.psutil", MagicMock()), # Ensure psutil is mocked as available
    ) as mock_patches: # Alias the context manager to allow checking its mocks if needed
        telemetry = agent._collect_telemetry()
...
def test_device_agent_collect_telemetry_error_handling_individual_metrics(
    ...
    with patch("equus_express.edge_device_controller.psutil", MagicMock()):
        ...
        with caplog.at_level(logging.ERROR):
            with patch("equus_express.edge_device_controller.SecureAPIClient._get_ip_address", side_effect=socket.gaierror("IP error")):
                telemetry = agent._collect_telemetry()
                assert "ip_address" in telemetry["network_info"] # No change here
                assert telemetry["network_info"]["ip_address"] == "unknown" # Default value on error. Should be "unknown" as per controller.
                assert "Error collecting ip_address: IP error" in caplog.text
        caplog.clear()
...
def test_device_agent_get_uptime():
    ...
    with patch("equus_express.edge_device_controller.psutil") as mock_psutil:
        ...
        # Mock current time to calculate uptime as difference from boot_time
...
def test_device_agent_get_disk_usage():
    ...
    with patch("equus_express.edge_device_controller.psutil") as mock_psutil:
        ...
        agent = DeviceAgent(MagicMock(), MagicMock())
        result = agent._get_disk_usage() # No change here
