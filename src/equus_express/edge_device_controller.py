...
try:
    ...
    smbus2 = None
    logging.warning(
        "smbus2 not installed, SMBus communication will be disabled."
    )
...
class SMBusNotAvailable(NotImplementedError):
    def __init__(
        self, message="smbus2 library is not available or bus not initialized."
    ):
        super().__init__(message)
...
class SecureAPIClient:
    def __init__(
        ...
        self.device_id = (
            device_id  # Will be set by _load_or_generate_keys if not provided
        )
    ...
    def _load_or_generate_keys(self):
        ...
        if (
            ...
            logger.info(
                f"Loaded existing keys and device ID: {self.device_id}"
            )
        else:
            ...
            if not self.device_id:
                ...
                logger.info(
                    f"Generated device ID from public key hash: {self.device_id}"
                )
    ...
    def _sign_request(
        self, method: str, endpoint: str, body: Optional[bytes] = None
    ) -> str:
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs):
        ...
        if "json" in kwargs and kwargs["json"] is not None:
            ...
            body_to_sign = kwargs[
                "data"
            ]  # Assuming data is already bytes or can be encoded

        headers["X-Signature"] = self._sign_request(
            method, endpoint, body_to_sign
        )
    ...
    def update_status(self, status: str, details: dict = None):
        ...
        logger.info(
            f"Updating status for device {self.device_id} to {status}..."
        )
        return self.post(
            "/api/device/status",
            json={
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Connection test for device {self.device_id} failed: {e}"
            )
            return False
...
class NATSClient:
    ...
    def _load_or_generate_nkeys(self):
        ...
        logger.warning(
            "NATS NKey generation/loading is a placeholder and needs proper implementation."
        )
    ...
    async def publish(self, subject: str, payload: bytes):
        ...
        if not self.nc:
            logger.warning(
                f"Attempted to publish to {subject} but NATS client is not connected."
            )
            return
        logger.info(
            f"Publishing to NATS subject '{subject}': {payload[:50]}..."
        )
        # Simulate publish
    ...
    async def subscribe(self, subject: str, cb):
        ...
        if not self.nc:
            logger.warning(
                f"Attempted to subscribe to {subject} but NATS client is not connected."
            )
            return
...
class DeviceAgent:
        ...
        if self.smbus_address is not None:
            if smbus2:
                try:
                    ...
                    logger.info(
                        f"SMBus initialized on bus {self.smbus_bus_num}."
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to initialize SMBus on bus {self.smbus_bus_num}: {e}"
                    )
                    self.smbus_bus = None
            else:
                logger.warning(
                    "smbus2 not installed, SMBus communication disabled."
                )
                self.smbus_bus = None
    ...
    async def start(self):
        ...
        try:
            ...
            self._command_listener_task = asyncio.create_task(
                self.nats_client.subscribe(
                    command_subject, self._handle_command_message
                )
            )
            logger.info(
                f"Subscribed to NATS command subject: {command_subject}"
            )

            logger.info("Device Agent started successfully.")
            await self.client.update_status(
                "online", {"message": "Device agent started."}
            )

        except Exception as e:
            logger.critical(f"Failed to start Device Agent: {e}")
            await self.client.update_status(
                "error", {"message": f"Failed to start: {e}"}
            )
    ...
    async def stop(self):
        ...
        await self.client.update_status(
            "offline", {"message": "Device agent stopped."}
        )
    ...
    async def run_telemetry_loop(self, interval: int = 60):
        ...
        while True:
            try:
                ...
                await self.client.update_status(
                    "warning", {"message": f"Telemetry skipped: {e}"}
                )
            except Exception as e:
                logger.error(f"Error collecting or sending telemetry: {e}")
                await self.client.update_status(
                    "warning", {"message": f"Telemetry error: {e}"}
                )
            finally:
    ...
    async def _handle_command_message(self, msg):
        ...
        try:
            ...
            logger.info(
                f"Command '{cmd_type}' executed successfully. Result: {result}"
            )
        except json.JSONDecodeError:
            response_payload = {
                "status": "error",
                "message": "Invalid JSON command format.",
            }
        ...
        if reply:
            await self.nats_client.publish(
                reply, json.dumps(response_payload).encode()
            )
    ...
    async def _handle_command(self, cmd_type: str, params: dict) -> dict:
        ...
        if cmd_type == "get_telemetry":
            ...
            if isinstance(new_interval, int) and new_interval > 0:
                ...
                return {
                    "message": f"Telemetry interval updated to {new_interval}s (requires restart to apply)."
                }
            else:
                raise ValueError(
                    "Invalid 'telemetry_interval' parameter. Must be a positive integer."
                )
            ...
            if not all(
                isinstance(x, int) for x in [address, command_code]
            ) or not isinstance(data, list):
                raise ValueError(
                    "Invalid parameters for smbus_write. Requires 'address' (int), 'command_code' (int), 'data' (list of int)."
                )
            ...
            if (
                not all(
                    isinstance(x, int) for x in [address, command_code, length]
                )
                or length <= 0
            ):
                raise ValueError(
                    "Invalid parameters for smbus_read. Requires 'address' (int), 'command_code' (int), 'length' (positive int)."
                )
            read_data = self._smbus_read_block_data(
                address, command_code, length
            )
            return {"data": list(read_data)}
        else:
            raise ValueError(f"Unknown command type: {cmd_type}")

    def _smbus_write_block_data(
        self, address: int, command_code: int, data: list
    ):
        ...
        try:
            ...
            logger.info(
                f"SMBus write: address=0x{address:02x}, command=0x{command_code:02x}, data={data}"
            )
        except Exception as e:
            logger.error(f"SMBus write failed: {e}")
            raise

    def _smbus_read_block_data(
        self, address: int, command_code: int, length: int
    ) -> bytearray:
        ...
        try:
            ...
            read_list = self.smbus_bus.read_i2c_block_data(
                address, command_code, length
            )
            read_data = bytearray(read_list)
            logger.info(
                f"SMBus read: address=0x{address:02x}, command=0x{command_code:02x}, length={length}, data={read_data.hex()}"
            )
            return read_data
        except Exception as e:
    ...
    def _collect_telemetry(self) -> dict:
        ...
        telemetry = {
            ...
            "network_info": {"ip_address": "0.0.0.0"},  # Default value
        ...
        try:
            telemetry["network_info"][
                "ip_address"
            ] = self.client._get_ip_address()
        except Exception as e:
...
async def main():
    ...
    DEVICE_ID = os.getenv(
        "DEVICE_ID"
    )  # Optional: if not set, client will generate one
    ...
    if SMBUS_ADDRESS:
        try:
            SMBUS_ADDRESS = (
                int(SMBUS_ADDRESS, 16)
                if SMBUS_ADDRESS.startswith("0x")
                else int(SMBUS_ADDRESS)
            )
        except ValueError:
            logger.error(
                f"Invalid SMBUS_ADDRESS: {SMBUS_ADDRESS}. Must be an integer or hex string."
            )
            SMBUS_ADDRESS = None
    ...
    try:
        ...
        logger.error(
            f"Invalid SMBUS_BUS_NUM: {SMBUS_BUS_NUM}. Must be an integer."
        )
        SMBUS_BUS_NUM = 1  # Default to bus 1

    # Initialize clients
    api_client = SecureAPIClient(
        base_url=API_BASE_URL, device_id=DEVICE_ID, key_dir=KEY_DIR
    )
    nats_client = NATSClient(
        nats_url=NATS_URL,
    ...
    try:
        ...
        while True:
            await asyncio.sleep(
                3600
            )  # Sleep for a long time, agent tasks run in background
    except asyncio.CancelledError:
        logger.info("Agent main loop cancelled.")
    except Exception as e:
        logger.critical(
            f"Agent encountered a critical error: {e}", exc_info=True
        )
    finally:
        await agent.stop()
